...
class BrowseAnimeForm(forms.Form):
    ...
    q = forms.CharField(label="Search", widget=forms.TextInput(
        attrs={'placeholder': 'Search'}), max_length=120, required=False)
    ...
    min_score = forms.FloatField(
        label="Min Score", min_value=0, max_value=10, required=False)
    max_score = forms.FloatField(
        label="Max Score", min_value=0, max_value=10, required=False)
